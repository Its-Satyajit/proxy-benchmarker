...
`);let y=[],S=Math.min(Math.max(o.concurrency||150,10),Math.max(d.length,1));if(d.length>0){let J=function(){let L=(Date.now()-w)/1e3,E=b>0?b/Math.max(L,.001):0,R=d.length-b,_=E>0?R/E:Number.NaN;te(`${s.cyan}Stage 2 (Verify)${s.reset} ${re(b,d.length)}% | ${b.toLocaleString()}/${d.length.toLocaleString()} | ${s.green}[OK] ${A} Alive${s.reset} | ${s.red}[FAIL] ${N} Dropped${s.reset} | ${oe(E)} | ETA ${O(_)}`)};var M=J;W.stdout.write(`${s.bold}${s.cyan}>> Stage 2: Health & Exit IP Verification (${S} parallel workers)${s.reset}
