...
`);let y=[],S=Math.min(Math.max(o.concurrency||150,10),Math.max(p.length,1));if(p.length>0){let q=function(){let L=(Date.now()-w)/1e3,E=f>0?f/Math.max(L,.001):0,R=p.length-f,W=E>0?R/E:Number.NaN;ee(`${s.cyan}Stage 2 (Verify)${s.reset} ${oe(f,p.length)}% | ${f.toLocaleString()}/${p.length.toLocaleString()} | ${s.green}[OK] ${A} Alive${s.reset} | ${s.red}[FAIL] ${N} Dropped${s.reset} | ${te(E)} | ETA ${D(W)}`)};var M=q;O.stdout.write(`${s.bold}${s.cyan}>> Stage 2: Health & Exit IP Verification (${S} parallel workers)${s.reset}
`);let w=Date.now(),f=0,A=0,N=0,Y=0;async function ne(){for(;;){let L=Y;if(L>=p.length)return;Y++;let E=p[L];if(E){try{let R=await Ft(E,t,o);f++,R.isAlive?(A++,y.push({proxy:E,exitIp:R.exitIp,endpointResults:R.endpointResults})):N++}catch{f++,N++}q()}}}let j=Array.from({length:S},()=>ne());await Promise.all(j),O.stdout.write(`
`);let B=((Date.now()-w)/1e3).toFixed(1);O.stdout.write(`  ${s.green}[OK] Stage 2 Finished in ${B}s${s.reset} - Verified ${s.bold}${A.toLocaleString()}${s.reset} alive proxies (${N.toLocaleString()} dropped)

`)}let P=[];if(y.length>0){let N=function(){let j=(Date.now()-w)/1e3,B=f>0?f/Math.max(j,.001):0,L=y.length-f,E=B>0?L/B:Number.NaN;ee(`${s.cyan}Stage 3 (Sites)${s.reset} ${oe(f,y.length)}% | ${f.toLocaleString()}/${y.length.toLocaleString()} | ${te(B)} | ETA ${D(E)}`)};var $=N;O.stdout.write(`${s.bold}${s.cyan}>> Stage 3: Top 50 Global Websites Benchmark (${y.length} alive proxies)${s.reset}
`);let w=Date.now(),f=0,A=0,Y=Math.min(Math.max(Math.floor(S/3),5),o.concurrency,y.length);async function q(){for(;;){let j=A;if(j>=y.length)return;A++;let B=y[j];if(!B)continue;let{proxy:L,exitIp:E,endpointResults:R}=B,W=[];if(o.benchmarkTopWebsites&&o.topWebsites.length>0)try{W=await Rt(L,o.topWebsites,o)}catch{W=[]}let se=R.filter(k=>k.ok),J=W.filter(k=>k.ok),Z=[...se,...J],F=Z.map(k=>k.totalLatencyMs),ae=Z.map(k=>k.connectTimeMs),ie=Z.map(k=>k.ttfbMs),ce=Z.map(k=>k.downloadSpeedBps),z=F.length>0?Math.round(F.reduce((k,H)=>k+H,0)/F.length):0,he=F.length>0?Math.min(...F):0,Qe=F.length>0?Math.max(...F):0,et=Bt(F),ge=ae.length>0?Math.round(ae.reduce((k,H)=>k+H,0)/ae.length):0,fe=ie.length>0?Math.round(ie.reduce((k,H)=>k+H,0)/ie.length):0,be=ce.length>0?Math.round(ce.reduce((k,H)=>k+H,0)/ce.length):0,X="UNKNOWN";E&&(n&&E===n?X="TRANSPARENT (LEAKING)":X="ELITE / ANONYMOUS");let Q="SLOW";z<400?Q="EXCELLENT":z<800?Q="GOOD":z<1500&&(Q="MODERATE");let K=o.topWebsites.length,tt=K>0?Number.parseFloat((J.length/K*100).toFixed(1)):0,_=K>0?J.length/K:1,ye=_*50,xe=Math.max(0,Math.min(20,20*(1-z/2500))),we=Math.max(0,Math.min(8,8*(1-he/1500))),ve=Math.max(0,Math.min(8,8*(1-ge/800))),Se=Math.max(0,Math.min(7,7*(1-fe/1500))),$e=Math.max(0,Math.min(7,be/(500*1024)*7)),ot=(xe+we+ve+Se+$e)*_,rt=Math.round(ye+ot),nt={websites:Math.round(ye),avgLatency:Math.round(xe*_),minLatency:Math.round(we*_),connectTime:Math.round(ve*_),ttfb:Math.round(Se*_),speed:Math.round($e*_),anonymity:X==="ELITE / ANONYMOUS"?10:0},st={proxy:L,status:"PASS",tier:Q,compositeScore:rt,scoreBreakdown:nt,exitIp:E,anonymity:X,endpointsTested:R.length,endpointsPassed:se.length,endpointsTotal:t.length,passRatePercent:Number.parseFloat((se.length/t.length*100).toFixed(1)),websitesTested:W.length,websitesPassed:J.length,websitesTotal:K,websitePassRatePercent:tt,avgLatencyMs:z,minLatencyMs:he,maxLatencyMs:Qe,medianLatencyMs:et,avgConnectTimeMs:ge,avgTtfbMs:fe,avgSpeedBps:be,firstFailedEndpoint:null,failureReason:null,endpointDetails:R,websiteDetails:W};r[L.protocol].push(L),P.push(st),f++,N()}}let ne=Array.from({length:Y},()=>q());await Promise.all(ne),O.stdout.write(`
...
`)}return P.sort((w,f)=>f.compositeScore!==w.compositeScore?f.compositeScore-w.compositeScore:w.avgLatencyMs-f.avgLatencyMs),P.forEach((w,f)=>{w.rank=f+1}),{results:r,benchmarks:P,stats:{total:e.length,completed:e.length,passed:P.length,failed:e.length-P.length,localPublicIp:n,durationSeconds:(Date.now()-m)/1e3,startedAt:new Date(m).toISOString(),completedAt:new Date().toISOString()}}}var I={trophy:'<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9H4.5a2.5 2.5 0 0 1 0-5H6"/><path d="M18 9h1.5a2.5 2.5 0 0 0 0-5H18"/><path d="M4 22h16"/><path d="M10 14.66V17c0 .55-.47.98-.97 1.21C7.85 18.75 7 20.24 7 22"/><path d="M14 14.66V17c0 .55.47.98.97 1.21C16.15 18.75 17 20.24 17 22"/><path d="M18 2H6v7a6 6 0 0 0 12 0V2Z"/></svg>',globe:'<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 2a14.5 14.5 0 0 0 0 20 14.5 14.5 0 0 0 0-20"/><path d="M2 12h20"/></svg>',shield:'<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.17 1.17 0 0 1 1.52 0C14.51 3.81 17 5 19 5a1 1 0 0 1 1 1z"/></svg>',settings:'<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12.22 2h-.44a2 2 0 0 0-2 2v.18a2 2 0 0 1-1 1.73l-.43.25a2 2 0 0 1-2 0l-.15-.08a2 2 0 0 0-2.73.73l-.22.38a2 2 0 0 0 .73 2.73l.15.1a2 2 0 0 1 1 1.72v.51a2 2 0 0 1-1 1.74l-.15.09a2 2 0 0 0-.73 2.73l.22.38a2 2 0 0 0 2.73.73l.15-.08a2 2 0 0 1 2 0l.43.25a2 2 0 0 1 1 1.73V20a2 2 0 0 0 2 2h.44a2 2 0 0 0 2-2v-.18a2 2 0 0 1 1-1.73l.43-.25a2 2 0 0 1 2 0l.15.08a2 2 0 0 0 2.73-.73l.22-.39a2 2 0 0 0-.73-2.73l-.15-.08a2 2 0 0 1-1-1.74v-.5a2 2 0 0 1 1-1.74l.15-.09a2 2 0 0 0 .73-2.73l-.22-.38a2 2 0 0 0-2.73-.73l-.15.08a2 2 0 0 1-2 0l-.43-.25a2 2 0 0 1-1-1.73V4a2 2 0 0 0-2-2z"/><circle cx="12" cy="12" r="3"/></svg>',copy:'<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg>',download:'<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/></svg>',search:'<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>',bolt:'<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M13 2 3 14h9l-1 8 10-12h-9l1-8z"/></svg>',rocket:'<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4.5 16.5c-1.5 1.26-2 5-2 5s3.74-.5 5-2c.71-.84.7-2.13-.09-2.91a2.18 2.18 0 0 0-2.91-.09z"/><path d="m12 15-3-3a22 22 0 0 1 2-3.95A12.88 12.88 0 0 1 22 2c0 2.72-.78 7.5-6 11a22.35 22.35 0 0 1-4 2z"/><path d="M9 12H4s.55-3.03 2-4c1.62-1.08 5 0 5 0"/><path d="M12 15v5s3.03-.55 4-2c1.08-1.62 0-5 0-5"/></svg>',plug:'<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22v-5"/><path d="M9 8V2"/><path d="M15 8V2"/><path d="M18 8v5a4 4 0 0 1-4 4h-4a4 4 0 0 1-4-4V8Z"/></svg>',clock:'<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>',gauge:'<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m12 14 4-4"/><path d="M3.34 19a10 10 0 1 1 17.32 0"/></svg>',alert:'<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><line x1="12" x2="12" y1="9" y2="13"/><line x1="12" x2="12.01" y1="17" y2="17"/></svg>',chevronRight:'<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg>',chevronDown:'<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>'};function je(){return`
    ...
    `}function Ke(e,t,o,n){let r=e.length>0?e[0]:null,i=e.length>0?Math.round(e.reduce((g,p)=>g+p.avgLatencyMs,0)/e.length):0,c=e.length>0?[...e].sort((g,p)=>g.avgLatencyMs-p.avgLatencyMs)[0]:null,m={http:{total:0,passed:0},https:{total:0,passed:0},socks4:{total:0,passed:0},socks5:{total:0,passed:0}};for(let g of e){let p=g.proxy.protocol;m[p]&&(m[p].total++,g.status==="PASS"&&m[p].passed++)}let l=JSON.stringify({generatedAt:t.completedAt||new Date().toISOString(),stats:t,localPublicIp:t.localPublicIp,endpoints:o,benchmarks:e}),h=je(),d=ze(l,JSON.stringify(I));return`<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
</html>`}import Ve from"node:fs/promises";import Ge from"node:path";import Dt from"node:process";async function re(e,t){let o=`${e}.${Dt.pid}.${Date.now()}.tmp`;await Ve.writeFile(o,t,"utf8"),await Ve.rename(o,e)}async function Ye(e,t){for(let[o,n]of Object.entries(t.outputFiles)){let r=e[o].map(De),i=r.length>0?`${r.join(`
`)}
`:"";await re(Ge.join(t.outputDir,n),i)}}async function qe(e,t){let o=JSON.stringify(e,null,2);await re(t,o)}function Je(e,t,o,n,r){a("========================================"),a("                 Results"),a("========================================"),a("");for(let i of["http","https","socks4","socks5"])a(`  ${i.toUpperCase().padEnd(7)} ${t[i].length.toLocaleString()} passed`);a(""),a(`  Local Network IP: ${e.localPublicIp||"Direct"}`),a(`  Verification    : Hard-failed if 0/${o} connected`),a(`  Top Websites    : ${u.topWebsites?.length||50} websites benchmarked`),a(`  Total tested    : ${e.total.toLocaleString()}`),a(`  Passed / Alive  : ${e.passed.toLocaleString()}`),a(`  Hard Failed     : ${e.failed.toLocaleString()} (excluded from report)`),a(`  Duration        : ${D(e.durationSeconds)}`),a(""),a("Output files:");for(let i of Object.values(u.outputFiles))a(`  ${Ge.join(u.outputDir,i)}`);a(""),a("Benchmark Reports:"),r.html&&a(`  ${s.green}HTML Report :${s.reset} ${r.html}`),r.json&&a(`  ${s.cyan}JSON Report :${s.reset} ${r.json}`),a("")}async function Ot(e){let t=[],o,n,r,i,c=!1;for(let m=0;m<e.length;m++){let l=e[m];if(l==="-h"||l==="--help"){c=!0;continue}if(l==="-n"||l==="--limit"){let d=e[++m];d&&(o=Number.parseInt(d,10));continue}if(l.startsWith("--limit=")){o=Number.parseInt(l.split("=")[1],10);continue}if(l==="-c"||l==="--concurrency"){let d=e[++m];d&&(n=Number.parseInt(d,10));continue}if(l.startsWith("--concurrency=")){n=Number.parseInt(l.split("=")[1],10);continue}if(l==="--tcp-concurrency"){let d=e[++m];d&&(r=Number.parseInt(d,10));continue}if(l.startsWith("--tcp-concurrency=")){r=Number.parseInt(l.split("=")[1],10);continue}if(l==="--safe"){i="safe";continue}if(l==="--home"){i="home";continue}if(l==="--turbo"||l==="--vps"){i="turbo";continue}if(l.startsWith("--preset=")){let d=l.split("=")[1].toLowerCase();(d==="safe"||d==="home"||d==="turbo")&&(i=d);continue}if(l.startsWith("-"))continue;try{if((await Ze.stat(l)).isFile()){let g=await Ze.readFile(l,"utf8"),p=Be(g);t.push(...p);continue}}catch{}let h=We(l);h.length>0&&t.push(...h)}return{customProxies:t,limit:o,concurrency:n,tcpConcurrency:r,preset:i,showHelp:c}}function Wt(){a(`
...
`)}async function _t(){let{customProxies:e,limit:t,concurrency:o,tcpConcurrency:n,preset:r,showHelp:i}=await Ot(U.argv.slice(2));i&&(Wt(),U.exit(0)),r==="safe"?(u.tcpConcurrency=35,u.concurrency=12,u.websiteConcurrency=3):r==="home"?(u.tcpConcurrency=80,u.concurrency=25,u.websiteConcurrency=5):r==="turbo"&&(u.tcpConcurrency=1200,u.concurrency=250,u.websiteConcurrency=15),o!==void 0&&(u.concurrency=o),n!==void 0&&(u.tcpConcurrency=n);let c=t!==void 0?t:u.limit;a(""),a("========================================"),a("   Proxy Benchmark & Best Network Finder "),a("========================================"),a(""),await Le(),a(`${s.green}[OK]${s.reset} Network tools ready`);let m=await Ee();a(`${s.cyan}[INFO]${s.reset} Origin Public IP: ${s.bold}${m||"Direct / Unknown"}${s.reset}`),a(`${s.gray}TCP Sockets: ${u.tcpConcurrency} | Workers: ${u.concurrency} | Timeout: ${u.timeoutSeconds}s | DNS: ${u.cloudflareDns} | Edge Targets: ${u.benchmarkTopWebsites?`${u.topWebsites?.length||50} sites`:"Disabled"}${s.reset}
`);let l=await Ae(u.testEndpoints,u);u.benchmarkTopWebsites&&u.topWebsites.length>0&&(u.topWebsites=await Ne(u.topWebsites,u));let h=[];if(e.length>0){a("========================================"),a("     Target Endpoints (CLI Input)       "),a("========================================"),a(""),h=Oe(e),a(`Loaded ${h.length} custom route(s) to benchmark:`);for(let b of h)a(`  ${s.cyan}${b.protocol.toUpperCase()}${s.reset}://${b.ip}:${b.port}`);a("")}else{a("========================================"),a("     Fetching Candidate Route Feeds     "),a("========================================"),a("");let{proxies:b}=await Fe(u);if(h=b,h.length===0)throw new Error("Candidate feeds contained no valid endpoints.");c>0&&c<h.length&&(a(`  Limiting benchmark to first ${c} candidates (LIMIT=${c})`),h=h.slice(0,c));let y={http:0,https:0,socks4:0,socks5:0};for(let S of h)y[S.protocol]++;a(`  Target candidate routes to benchmark (${h.length.toLocaleString()} total):
...
`);let{results:d,benchmarks:g,stats:p}=await Ue(h,l,u,m);a("========================================"),a("       Generating Reports & Files       "),a("========================================"),a(""),await Ye(d,u);let v=Xe.join(u.outputDir,u.reportFiles.html),T=Ke(g,p,l,v);await re(v,T);let x=Xe.join(u.outputDir,u.reportFiles.json);await qe({generatedAt:p.completedAt,stats:p,endpoints:l,benchmarks:g},x),Je(p,d,l.length,u.testEndpoints.length,{html:v,json:x})}U.on("SIGINT",()=>{U.stdout.write(`
